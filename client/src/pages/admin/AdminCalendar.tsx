...
import { Calendar, Clock, ChevronLeft, ChevronRight, Video, User, Grid3X3, List } from "lucide-react";
...
export default function AdminCalendar() {
  ...
  const { data: appointments, loading: appointmentsLoading } = useFirestoreCollection<Appointment>(
    "appointments",
    [orderBy("date", "desc")]
  );

  // Fetch all users to get client names
  const { data: users, loading: usersLoading } = useFirestoreCollection<UserType>(
    "users",
    [where("role", "==", "client")]
  );
  ...
  // Get client name for appointment
  const getClientName = (appointment: any) => {
    const client = users?.find(u => u.uid === appointment.userId);
                    ...
                    return (
                      <div
                        ...
                        className={`
                          ...
                          ${hasAppt ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800' : ''}
                        `}
                      >
                        ...
                        <div className="flex-1 space-y-1">
                          {appointments.slice(0, 2).map((apt, aptIndex) => (
                            <div
                              key={aptIndex}
                              className="text-xs bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200 px-1 py-0.5 rounded truncate"
                              title={`${apt.timeslot} - ${getClientName(apt)}`}
                            >
                              {apt.timeslot}
                            </div>
                          ))}
                            ...
                            return (
                              ...
                              >
                                {slotAppointments.map((apt, idx) => (
                                  <div
                                    key={idx}
                                    className="text-xs px-2 py-1 rounded bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200 mb-1 truncate"
                                    title={`${apt.timeslot} - ${getClientName(apt)}`}
                                  >
                                    {apt.timeslot}
                                  </div>
