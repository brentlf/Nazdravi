...
export const onUserCreated = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    if (user && user.email && user.name && user.role === 'client') {
      ...
      console.log(`Email ${user.email ? 'sent successfully' : 'failed'} to ${user.email}`);
    }
  });
...
// 4. Process Mail Queue - Updated to handle all email types including welcome and payment reminder
